...
function setFixedHeaderToTable() {
  var table = $('table[data-fixed-header]');
  if (table) {
...
$(window)
  .resize(function() {
    ...
    if ($('#art-templates')
      ...
      if ($(window)
        ...
        $('#art-templates')
          .hide();
        $('#art-templates')
          .removeClass("large-padding-bottom-y");
        $('#add-art-not-avail')
          .show();
      } else if ($(window)
        .width() > 1000) {
        $('#art-templates')
          .show();
        $('#art-templates')
          .addClass("large-padding-bottom-y");
        $('#add-art-not-avail')
          .hide();
      }
    }
    ...
    if ($('#og_art_filter_wrapper')
      .length) {
      if ($(window)
        .width() <= 1000) {
        $('#og_art_filter_wrapper')
          .hide();
        $('.toggleFilterPanel')
          .show();
        $('.toggleFilterPanelUp')
    ...
    if ($('.related-events-max-four')
      .length) {
      calcRelatedEventsWidth();
    }
    if ($('.related-events-over-four')
      .length) {
      calcRelatedEventsSliderWidth();
    }
    calcViewEventsHeight();
...
var dropMenu = function() {
  ...
  $trigger.on('click', function(e) {
    ...
    if ($(".mob_menu")
      .hasClass("mob_menu_active")) {
      $('.mob_menu')
        .removeClass("mob_menu_active");
    }

    // remove active styling of other triggers and dropdowns
    var $data = $(this)
      .attr("trigger");
    $drop.not('[dropdown= ' + $data + ']')
      .removeClass("active");
    $trigger.not($(this))
      .removeClass("active");
...
var openRegPanel = function() {
  ...
  if ($("[data-tab]")
    ...
    e.click(function() {
      ...
      $('.popup-inner.registration-popup')
        .removeClass("artist-rego");
      if (t != "register") {
...
var dashMenu = function() {
  if ($('.navigation_toggle_dots')
    .length) {
    $(".navigation_toggle_dots")
      .click(function(e) {
    ...
    reader.onload = function(e) {
      ...
      getOrientation(input.files[0], function(orientation) {
        ...
        if ([5, 6, 7, 8].indexOf(orientation) > -1) {
          ...
          c.width = $('#profileRotator')
            .height();
          c.height = $('#profileRotator')
            .width();
          var ctx = c.getContext("2d");
          ctx.transform(0, 1, -1, 0, $('#profileRotator')
            .height(), 0);
          ctx.drawImage(document.getElementById('profileRotator'), 0, 0);
          urlRot = c.toDataURL();
          // set Base64 string in src of positioner
          $('#profile-image-preview')
            .css("background-image", "url(" + urlRot + ")");
        } else {
          // set Base64 string in src of positioner
          $('#profile-image-preview')
            .css("background-image", "url(" + binImg + ")");
        }
      });
    };
    ...
    reader.onload = function(e) {
      ...
      getOrientation(input.files[0], function(orientation) {
        ...
        if ([5, 6, 7, 8].indexOf(orientation) > -1) {
          ...
          c.width = $('#event-photo-rotator')
            .height();
          c.height = $('#event-photo-rotator')
            .width();
          var ctx = c.getContext("2d");
          ctx.transform(0, 1, -1, 0, $('#event-photo-rotator')
            .height(), 0);
...
var saveNewCoverPic = function() {
  if ($('#saveProfile')
    ...
    if ($("#cover-img")
      ...
      var imgWidth = $("#cover-img")
        .width();
      var imgHeight = $("#cover-img")
        .height();

      // get transform matrix
      var imgTrans = $("#cover-img")
        .css("transform");
      var matrix = new CSSMatrix(imgTrans);

      // get size of cover image parent element
      var wrapWidth = $('.profile-cover')
        .width();
      var wrapHeight = $('.profile-cover')
        .height();
    ...
    reader.onload = function(e) {
      ...
      getOrientation(input.files[0], function(orientation) {
        ...
        if ([5, 6, 7, 8].indexOf(orientation) > -1 && [5, 6, 7, 8].indexOf(orientation) !== 3) {
          $('#original-art-rotator')
            .attr('src', binImg);
          var c = document.getElementById("original-art-slice");
          c.width = $('#original-art-rotator')
            .height();
          c.height = $('#original-art-rotator')
            .width();
          var ctx = c.getContext("2d");
          ctx.transform(0, 1, -1, 0, $('#original-art-rotator')
            .height(), 0);
          ...
          c2.width = $('#original-art-rotator')
            .height();
          c2.height = $('#original-art-rotator')
            .width();
          var ctx2 = c2.getContext("2d");
          ctx2.transform(0, 1, -1, 0, $('#original-art-rotator')
            .height(), 0);
          ctx2.rotate(Math.PI);
          ...
          $('#original-art-preview')
            .css("background-image", "url(" + urlRot2 + ")");
        } else {
          // set Base64 string in src of positioner
          $('#original-art-preview')
            .css("background-image", "url(" + binImg + ")");
...
var showAdditionalAddress = function() {
  ...
  if ($('#primaryAddress')
    ...
    $("#primaryAddress")
      .click(function(e) {
        if ($("#billingAddressRow")
          .is(":visible")) {
          $("#billingAddressRow")
            .hide();
        }
        if ($("#billingCountryStateRow")
          .is(":visible")) {
          $("#billingCountryStateRow")
            .hide();
        }
        if ($("#billingAddressCityRow")
          .is(":visible")) {
          $("#billingAddressCityRow")
            .hide();
        }
      });
    $("#billingAddress")
...
var getRecentViewsSliderCoeff = function() {
  var coeff;
  var numItems = $(".product-show > .column-4")
    .length;
...
var getRecentViewMove = function() {
  var a = $(".product-show > .column-4")
    .outerWidth(true) + "px";
  return a;
};
/************ returns sliderLimit value for right and left button clicks ******************************/
var getRecentViewSliderLimit = function() {
  var b = -($(".product-show > .column-4")
    .outerWidth(true) * getRecentViewsSliderCoeff());
  return b;
};
  ...
  if ($(".right-arrow")
    ...
    $(".right-arrow")
      .click(function() {
        ...
        if (currentPosition >= getRecentViewSliderLimit()) show.stop(false, true)
          .animate({
...
var masonryEventWorks = function() {
  if ($('.events-works-wrapper')
    .length) {
    $('.events-works-wrapper')
      .masonry({
        // options
  ...
  if ($('.related-events-over-four')
    ...
    if ($(window)
      ...
      if (getMobileOperatingSystem() == "Android" || getMobileOperatingSystem() == "Windows Phone") {
        $('.product-show .column-4.profile-event')
          .css('width', '198px');
        $('.related-events-over-four .product-view-container')
          .css('width', '218px');
...
var calcRelatedEventsWidth = function() {
  if ($('.related-events-max-four')
    ...
    if ($(window)
      ...
      $('.related-events-max-four .column-4.profile-event')
        .each(function(index) {
          relatedEventsWidth = relatedEventsWidth + $(this)
            .outerWidth(true);
        });
      $('.related-events-max-four')
        .css("width", relatedEventsWidth);
    } else {
      $('.related-events-max-four')
        .css("width", "100%");
    }
...
var calcViewEventsHeight = function() {
  if ($('.view-current-events')
    .length) {
    var numEvents = $('.profile-event')
      .length;
    var eventHeight = $('.profile-event')
      .outerHeight(true);
    var eventsWidth = $('.view-current-events')
      .outerWidth(true);
    $('.view-current-events-scroll')
      .css("width", eventsWidth + 18);
    if (numEvents <= 4) {
      $('.view-current-events')
        .css("height", eventHeight);
      $('.view-current-events-scroll')
        .css("height", eventHeight);
    } else if (numEvents > 4 && numEvents <= 8) {
      $('.view-current-events')
        .css("height", eventHeight * 2);
      $('.view-current-events-scroll')
        .css("height", eventHeight * 2);
    } else if (numEvents >= 9) {
      $('.view-current-events')
        .css("height", eventHeight * 3);
      $('.view-current-events-scroll')
        .css("height", eventHeight * 3);
    }
    if (numEvents > 12) {
...
var calcViewProductsHeight = function() {
  if ($('#view-products-select')
    .length) {

    var numEvents = $('.box-product3')
      .length;
    var eventHeight = $('.box-product3')
      .outerHeight(true);
    var eventsWidth = $('.view-current-events')
      .outerWidth(true);
    $('.view-current-events-scroll')
...
var calcViewFollowsHeight = function() {
  if ($('.view-follows')
    ...
    var numFollows = $('.follow-item-col')
      .length;
    var followsHeight = $('.follow-item-col')
      .outerHeight(true);
    var followsWidth = $('.view-follows')
      .outerWidth(true);
    $('.view-follows-scroll')
      .css("width", followsWidth + 18);

    if (numFollows <= 5) {
      $('.view-follows')
        .css("height", followsHeight);
      $('.view-follows-scroll')
        .css("height", followsHeight);
    } else if (numFollows > 5 && numFollows <= 10) {
      $('.view-follows')
        .css("height", followsHeight * 2);
      $('.view-follows-scroll')
        .css("height", followsHeight * 2);
    } else if (numFollows > 10 && numFollows <= 15) {
      $('.view-follows')
        .css("height", followsHeight * 3);
      $('.view-follows-scroll')
        .css("height", followsHeight * 3);
    } else if (numFollows > 15 && numFollows <= 20) {
      $('.view-follows')
        .css("height", followsHeight * 4);
      $('.view-follows-scroll')
        .css("height", followsHeight * 4);
    ...
    if (numFollows > 5 && $(window)
      .width() < 1000 && $(window)
      .width() > 509) {
      bounceDownArrow();
    } else if (numFollows > 4 && $(window)
      .width() < 509) {
      bounceDownArrow();
    }
  }
...
var calcViewGalleriesHeight = function() {
  if ($('.view-galleries')
    .length) {
    var numGalls = $('.view-box-gallery')
      .length;
    var eventHeight = $('.view-box-gallery')
      .outerHeight(true);
    var eventsWidth = $('.view-galleries')
      .outerWidth(true);
    $('.view-galleries-scroll')
      .css("width", eventsWidth + 18);
    if (numGalls <= 3) {
      $('.view-galleries')
        .css("height", eventHeight);
...
var calcViewInvitesHeight = function() {
  ...
  if ($('.view-invites-sent')
    .length) {
    numInvitesSent = $('.view-invites-sent .artist-invite')
      .length;

    if ($('.view-invites-sent')
      .is(':visible')) {
      invitesSentHeight = $('.view-invites-sent .artist-invite')
        .outerHeight(true);
      invitesSentWidth = $('.view-invites-sent')
        .outerWidth(true);
    } else {
      invitesSentHeight = $('.view-invites-received .artist-invite')
        .outerHeight(true);
      invitesSentWidth = $('.view-invites-received')
        .outerWidth(true);
    }

    $('.view-invites-sent-scroll')
      .css("width", invitesSentWidth + 18);

    if (numInvitesSent <= 5) {
      for (i = 1; i < 5; i++) {
        if (numInvitesSent == i) {
          $('.view-invites-sent')
            .css("height", invitesSentHeight * i);
          $('.view-invites-sent-scroll')
            .css("height", invitesSentHeight * i);
        }
      }
      ...
      $('.view-invites-sent')
        .css("height", invitesSentHeight * 5);
      $('.view-invites-sent-scroll')
        .css("height", invitesSentHeight * 5);
      bounceDownArrow();
    }
  ...
  if ($('.view-invites-received')
    .length) {

    var numInvitesRecd = $('.view-invites-received .artist-invite')
      .length;
    ...
    if ($('.view-invites-received')
      .is(':visible')) {
      invitesRecdHeight = $('.view-invites-received .artist-invite')
        .outerHeight(true);
      invitesRecdWidth = $('.view-invites-received')
        .outerWidth(true);
    } else {
      invitesRecdHeight = $('.view-invites-sent .artist-invite')
        .outerHeight(true);
      invitesRecdWidth = $('.view-invites-sent')
        .outerWidth(true);
    }
    $('.view-invites-received-scroll')
      .css("width", invitesRecdWidth + 18);
    if (numInvitesRecd <= 5) {
      for (i = 1; i < 5; i++) {
        if (numInvitesSent === i) {
          $('.view-invites-received')
            .css("height", invitesRecdHeight * i);
          $('.view-invites-received-scroll')
            .css("height", invitesRecdHeight * i);
        }
      }
      ...
      $('.view-invites-received')
        .css("height", invitesRecdHeight * 5);
      $('.view-invites-received-scroll')
        .css("height", invitesRecdHeight * 5);
      bounceDownArrowRecd();
    }
...
var showMyProdBtns = function() {
  $(".product-img2.my-prod-img")
    .mouseover(function() {
      $(this)
        .parent()
        .find(".my-product-buttons-wrapper")
        .show();
    });
  $(".product-img2.my-prod-img")
    .mouseout(function() {
...
var showProductDesc = function(e) {
  ...
  $(".my-product-description-box")
    .each(function() {
      if ($(this)
        .attr("id") != $("#descriptionbox_" + id)) {
        $(this)
          .hide();
      }
    });
...
var hoverOnProductButtons = function() {
  ...
  $('.my-product-buttons-wrapper')
    .mouseout(function() {
      $(this)
        .parent()
        .attr('onclick', 'showProductDesc(this)');
    });
};
...
var hideDeleteConfirm = function(e) {
  $("#product-delete-confirmation-popup")
    .hide();
  var p_box = $("#delete_prod_id")
    .val();
  $('#' + p_box)
    .attr("onclick", "");
  $('#' + p_box)
    .html('<div class="overlay_product_delete"><h5>Pending deletion</h5></div>');
  $('#' + p_box)
    .css('cursor', 'auto');
  if ($('#' + p_box)
    .parent()
...
var previewBackProduct = function(e) {
  ...
  var lg_bg = $(e)
    ...
    .css("background");
  $(e)
    .css("background", lg_bg);
  $('#bgPreviewImgVal')
    .val(bg_img);
  $(e)
    .parent()
    .find('.pdb-main-img')
    .css("background", $('#bgPreviewImgVal')
      .val());

};
var printDimensions = function() {
  if ($(".art-print-product-home")
    .length) {
    $(".art-print-product-home")
      .each(function(index) {
...
var masonryCatOgArtWorks = function() {
  if ($('.cat-ogart-works-wrapper')
    .length) {
    $('.cat-ogart-works-wrapper')
      .masonry({
        // options
...
var showCategoryProdButtons = function() {
  $(".cat-ogart-work-container .product-img")
    .mouseover(function() {
      $(this)
        .find(".my-product-cat-buttons-wrapper")
        .show();
    });
  $(".cat-ogart-work-container .product-img")
    .mouseout(function() {
...
var showProdCatButtonsMobile = function() {
  if ($(".cat-ogart-work-container")
    .length) {
    if ($(window)
      .width() <= 1000) {
      $(".cat-ogart-work-container")
        .find(".my-product-cat-buttons-wrapper")
        .show();
      $(".cat-ogart-work-container .product-img")
        .off('mouseout');
...
var showCategoryProdButtonsAll = function() {
  if ($(".product-cat-wrapper")
    .length) {
    $(".column-3.box-product3")
      .mouseover(function() {
        $(this)
          .find(".my-product-cat-buttons-wrapper")
          .show();
...
var quantitySpinner = function() {
  ...
  if ($('#prod-shipping-time')
    .length) {
    var spinner2 = $("#prod-shipping-time")
      .spinner({
        min: 1,
        max: 1000
      });
  }
  if ($('#product-quantity-cards')
    .length) {
    spinner = $("#product-quantity-cards")
      .spinner({
        min: 1,
        max: 2
      });
...
var previewBackGreetingIndividual = function(e) {
  ...
  if (portrait === true) {
    $("#greeting")
      .attr("style", 'background: url("images/greetingcard-back-portrait-blank.png") no-repeat center 42% / auto 68%, linear-gradient( #eeebeb, #f9f9f9); position: relative;');
  } else if (portrait === false) {
    $("#greeting")
      .attr("style", 'background: url("images/greetingcard-back-blank.png") no-repeat 55% 49% / 68.5% auto, linear-gradient( #eeebeb, #f9f9f9); position: relative;');
  }
...
var quantitySpinnerCartCards = function() {
  if ($('.cart-product-quantity-cards')
    .length) {
    $('[id^="cpqc_"]')
      .each(function(index) {
        var spinner = $(this)
...
var printPaper = function(e) {
  ...
  if (e.value === "Canvas") {
    if ($(window)
      .width() >= 1000) {
      $('#indv-print-container')
        .css("background", "url('images/print-view.png') no-repeat center center / 100% 100%");
      $('#print-main')
        .addClass("print-medium-selected");
      $('#print-main')
        .addClass("print-medium-canvas");
    }
    $('#print-main')
      .css("border", "none");
    $('#print-size')
    ...
    $('#print-frame')
      .prop('disabled', false);
    $('#print-main')
      .removeClass("print-medium-canvas");
    ...
    if ($(window)
      .width() >= 1000) {
      $('#indv-print-container')
        .css("background", "linear-gradient( #eeebeb, #f9f9f9)");
      $('#print-main')
        .animate({
          width: $('#init-img-width')
            .val(),
          height: $('#init-img-height')
            .val()
        });
      $('#print-main')
        .removeClass("print-medium-selected");
      $('#print-main')
        .removeClass("print-medium-canvas");
      $('#print-main')
        .css("margin-left", 0);
...
var printSize = function(e) {
  if ($(window)
    ...
    if ($(window)
      ...
      if (e.value === "select") {
        $('#print-main')
          .animate({
            ...
            $('#print-main')
              .animate({
                marginLeft: 0 - ($('#print-main')
                  .outerWidth() / 2)
              }, 200);
          });
      } else if (e.value === "6 x 4in (152 x 101mm)") {
        $('#print-main')
          .animate({
            width: "44px",
            height: "29px",
      ...
      } else if (e.value === "7 x 5in (178 x 127mm)") {
        $('#print-main')
          .animate({
            width: "51px",
            height: "36px",
      ...
      } else if (e.value === "A5 (210 x 148mm)") {
        $('#print-main')
          .animate({
            width: "42px",
            height: "60px",
      ...
      } else if (e.value === "8 x 8in (200 x 200mm)") {
        $('#print-main')
          .animate({
            width: "58px",
            height: "58px",
      ...
      } else if (e.value === "6 x 12in (152 x 303mm)") {
        $('#print-main')
          .animate({
            width: "44px",
            height: "87px",
      ...
      } else if (e.value === "10 x 8in (250 x 200mm)") {
        $('#print-main')
          .animate({
            width: "72px",
            height: "58px",
      ...
      } else if (e.value === "12 x 8in (300 x 200mm)") {
        $('#print-main')
          .animate({
            width: "87px",
            height: "58px",
      ...
      } else if (e.value === "A4 (297 x 210mm)") {
        $('#print-main')
          .animate({
            width: "60px",
            height: "85px",
      ...
      } else if (e.value === "6 x 18in (152 x 457mm)") {
        $('#print-main')
          .animate({
            width: "44px",
            height: "130px",
      ...
      } else if (e.value === "8 x 16in (203 x 406mm)") {
        $('#print-main')
          .animate({
            width: "58px",
            height: "116px",
      ...
      } else if (e.value === "12 x 12in (305 x 305mm)") {
        $('#print-main')
          .animate({
            width: "87px",
            height: "87px",
      ...
      } else if (e.value === "14 x 11in (355 x 280mm)") {
        $('#print-main')
          .animate({
            width: "101px",
            height: "80px",
      ...
      } else if (e.value === "8 x 24in (203 x 610mm)") {
        $('#print-main')
          .animate({
            width: "58px",
            height: "173px",
      ...
      } else if (e.value === "16 x 12in (406 x 305mm)") {
        $('#print-main')
          .animate({
            width: "116px",
            height: "87px",
      ...
      } else if (e.value === "A3 (420 x 297mm)") {
        $('#print-main')
          .animate({
            width: "85px",
            height: "120px",
      ...
      } else if (e.value === "16 x 16in (406 x 406mm)") {
        $('#print-main')
          .animate({
            width: "116px",
            height: "116px",
      ...
      } else if (e.value === "12 x 24in (305 x 610mm)") {
        $('#print-main')
          .animate({
            width: "87px",
            height: "173px",
      ...
      } else if (e.value === "20 x 16in (508 x 406mm)") {
        $('#print-main')
          .animate({
            width: "144px",
            height: "116px",
      ...
      } else if (e.value === "A2 (594 x 420mm)") {
        $('#print-main')
          .animate({
            width: "120px",
            height: "169px",
      ...
      } else if (e.value === "24 x 16in (610 x 406mm)") {
        $('#print-main')
          .animate({
            width: "173px",
            height: "116px",
      ...
      } else if (e.value === "20 x 20in (508 x 508mm)") {
        $('#print-main')
          .animate({
            width: "144px",
            height: "144px",
      ...
      } else if (e.value === "12 x 36in (305 x 915mm)") {
        $('#print-main')
          .animate({
            width: "87px",
            height: "173px",
      ...
      } else if (e.value === "Poster (700 x 500mm)") {
        $('#print-main')
          .animate({
            width: "199px",
            height: "142px",
      ...
      } else if (e.value === "20 x 30in (508 x 762mm)") {
        $('#print-main')
          .animate({
            width: "144px",
            height: "216px",
      ...
      } else if (e.value === "A1 (840 x 594mm)") {
        $('#print-main')
          .animate({
            width: "169px",
            height: "238px",
      ...
      } else if (e.value === "24 x 36in (610 x 915mm)") {
        $('#print-main')
          .animate({
            width: "173px",
            height: "260px",
  ...
  if ($(window)
    ...
    if (e.value === "select") {
      ...
      $('#print-main')
        .css("padding", "0");
      $('#print-main')
        .css("margin-left", m_left);
    } else if (e.value === "POL4033") {
      $('#print-main')
        .css("border", "7px solid #000");
    } else if (e.value === "POL4033") {
      $('#print-main')
        .css("border", "10px solid #000");
    } else if (e.value === "POL4279") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35oak-large-img.jpg") 7 round');
    } else if (e.value === "POL4604") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35oak-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    } else if (e.value === "0342BK") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35black-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344BK") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35black-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    } else if (e.value === "0342GY") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35grey-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344GY") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35grey-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
      ...
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35palegrey-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344PG") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35palegrey-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    } else if (e.value === "0342WH") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35white-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344WH") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35white-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    } else if (e.value === "0342IV") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35ivory-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344IV") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35ivory-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    } else if (e.value === "0342WA") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "7px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35walnut-large-img.jpg") 7 round');
      //$('#print-main').css("margin-left", m_left2 - 7);
    } else if (e.value === "0344WA") {
      $('#print-main')
        .css("border-style", "solid");
      $('#print-main')
        .css("border-width", "10px");
      $('#print-main')
        .css("border-image", 'url("images/pattern/35walnut-large-img.jpg") 10 round');
      //$('#print-main').css("margin-left", m_left2 - 10);
    }
    $('#print-main')
      .animate({
...
var showSearchWl = function() {
  if ($(".view-products-search")
    .length) {
    $(".column-4.box-product3")
      .mouseover(function() {
        $(this)
          .find(".my-product-cat-buttons-wrapper")
          .show();
...
var changeSupportersView = function(e) {
  var view = document.getElementById("select-supporters-view")
    .value;
  if (view === "Supporters") {

    $('.view-follow-supporters')
      .show();
    $('.view-follow-graphs')
      .hide();
    calcViewFollowsHeight();
...
var changeSuppTimeQuarter = function() {
  var qval = document.getElementById("graphsSuppTimeQ")
    .value;
  if (qval === "Q1") {
    $('#q1')
      .show();
...
var changeSuppViewQuarter = function() {
  var qval = document.getElementById("suppViewsQ")
    .value;
  if (qval === "Q1") {
    $('#qv1')
      .show();
...
var changeSuppSpendQuarter = function() {
  var qval = document.getElementById("suppSpendQ")
    .value;
  if (qval === "Q1") {
    $('#qs1')
      .show();
...
var colorSuppMap = function() {
  if ($('#world-map')
    ...
    jQuery.each(countryList, function(idx, value) {
      $("#topCountryList")
        .append('<li><div class="countryDot"></div><p>' + value + '</p></li>');
    });
  }
};
...
$(function() {
  $('.page-footer__faq .right .fullArticle .article-video a')
    .each(function() {
 ...
 * ZORAN START
 */
var changeColorStartup = function() {
  if ($('#order-table').length) {
    $("#order-table").children('tbody').children('tr').each(function() {
      var val = $(this).children('td').find('select').val();
      switch (val) {
        case '1':
          $(this).children('td').find('select').addClass('warning');
          break;
        case '2':
          $(this).children('td').find('select').addClass('pending');
          break;
        case '3':
          $(this).children('td').find('select').addClass('default');
          break;
        case '4':
          $(this).children('td').find('select').addClass('success');
          break;
      }
    });
  }
